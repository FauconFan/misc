...
/*   Updated: 2017/08/24 09:50:27 by jpriou           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				main(int argc, char **argv)
{
	...
	if (argc <= 3)
	{
		ft_putstr("Tail usage : ./ft_tail -c 100 name_file...\n");
		return (0);
	}
...
void			display_main_file(char *name_file, int to_ignore, int mode)
{
	...
	real_ignore = (mode == MODE_FROM_HEAD) ? to_ignore - 1:
		get_size_file(name_file) - to_ignore;
	if ((file_descriptor = open(name_file, O_RDONLY)) == -1)
		return ;
	ret = read(file_descriptor, buffer, MAX0(MIN(real_ignore)));
	if (ret == -1)
		return ;
	while (ret != 0)
	{
