...
let rec param config =
  ...
  let text_layer = new SLAC.layer [(text_zone :> SLAC.acomponent)] in
...
and
  ...
  let button_check_solve = new button (180,60) ["Check_solve"] (w - 230, 700) (fun _ -> GMessage.Apply fun_check_solve) in
  let button_check = new button (180,60) ["Check"] (w - 230, 600) (fun _ -> GMessage.Apply fun_check) in
  ...
  let button_red = new button ~color_font:(rgb 255 0 0) (70,70) [] (w - 215, 200) (fun _ -> GMessage.Apply (fun _ -> print_endline "RED")) in
  let button_blue = new button ~color_font:(rgb 0 0 255) (70,70) [] (w - 135, 200) (fun _ -> GMessage.Apply (fun _ -> print_endline "BLUE")) in
  let button_white = new button ~color_font:(rgb 255 255 255) (70,70) [] (w - 175, 120) (fun _ -> GMessage.Apply (fun _ -> print_endline "WHITE")) in
