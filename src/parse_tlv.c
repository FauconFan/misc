...
static uint16_t parse_goaway(uint8_t * tlv) {
    ...
    if (len < 1) {
        ...
        switch (code) {
            case 0: dprintf(ui_getfd(), "unknown reason"); break;
            case 1: dprintf(ui_getfd(), "sender quit the system"); break;
            case 2: dprintf(ui_getfd(), "hello message missing for too long"); break;
            case 3: dprintf(ui_getfd(), "sender didn't respect the protocol"); break;
            case 4: dprintf(ui_getfd(), "sender has too many neighbours"); break;
            default: dprintf(ui_getfd(), "unknown code"); break;
        }
        msg = tlv + 3;
