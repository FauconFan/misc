...
uint64_t		eval(const t_packet &pack) {
	uint64_t	answer;

	switch (pack.type) {
		case 0:
			answer = 0;
			for (const t_packet &subpacket: pack.subpackets) {
				answer += eval(subpacket);
			}
			break;
		case 1:
			answer = 1;
			...
			break;
		case 2:
			answer = eval(pack.subpackets[0]);
			for (size_t i = 1; i < pack.subpackets.size(); ++i) {
				answer = min(answer, eval(pack.subpackets[i]));
			}
			break;
		case 3:
			answer = eval(pack.subpackets[0]);
			for (size_t i = 1; i < pack.subpackets.size(); ++i) {
				answer = max(answer, eval(pack.subpackets[i]));
			}
			break;
		case 4:
			answer = pack.value;
			break;
		case 5:
			if (eval(pack.subpackets[0]) > eval(pack.subpackets[1])) answer = 1;
			else answer = 0;
			break;
		case 6:
			if (eval(pack.subpackets[0]) < eval(pack.subpackets[1])) answer = 1;
		...
		case 7:
			...
			break;
		default:
			cout << "unsupported type operation" << endl;
			break;
	}
