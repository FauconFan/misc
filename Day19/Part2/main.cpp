...
uint64_t			point_manhattan_distance(const Point &p1, const Point &p2) {
	uint64_t	res = 0;
...
int		main(void) {
	...
	while (isFound.size() != pb.scanners.size()) {
		...
		for (size_t i = 0; i < pb.scanners.size(); ++i) {
			...
			if (resOverlap) {
				cout << "overlap found with scanner " << i << " at " << resOverlap->offset << endl;
				...
				isFound.insert(i);
				positionScanners.insert(resOverlap->offset);
				break;
			}
		}
	}

	for (const Point &p1: positionScanners) {
		for (const Point &p2: positionScanners) {
			answer = max(answer, point_manhattan_distance(p1, p2));
		}
	}
