...
using Signature = array<E_Amph, 19>;

struct		State
{
	array<E_Amph, 11>			hallway;
	array<array<E_Amph, 2>, 4>	rooms;
	uint64_t					cost = 0;
	...
	bool	isFinished() const {
		...
		if (rooms[0][0] != AMPH_A) return false;
		if (rooms[0][1] != AMPH_A) return false;

		if (rooms[1][0] != AMPH_B) return false;
		if (rooms[1][1] != AMPH_B) return false;

		if (rooms[2][0] != AMPH_C) return false;
		if (rooms[2][1] != AMPH_C) return false;

		if (rooms[3][0] != AMPH_D) return false;
		if (rooms[3][1] != AMPH_D) return false;
...
vector<State>	computeNextStatesForHallway(const State &s) {
	...
	for (const size_t pos: liPos) {
		...
		cost += 1;
		if (s.rooms[posRoom][1] == NONE) cost += 1;

		cost *= getCostFromAmph(amph);

		current.hallway[pos] = NONE;
		if (current.rooms[posRoom][1] == NONE) current.rooms[posRoom][1] = amph;
		else current.rooms[posRoom][0] = amph;
...
vector<State>	computeNextStatesForRooms(const State &s) {
	...
	for (size_t i = 0; i < s.rooms.size(); ++i) {
		...
		if (s.rooms[i][0] == expectedAmph && s.rooms[i][1] == expectedAmph) continue;
		if (s.rooms[i][0] == NONE && s.rooms[i][1] == expectedAmph) continue;
		if (s.rooms[i][0] != NONE) {
			liPos.push_back({i, 0});
		} else if (s.rooms[i][1] != NONE) {
			liPos.push_back({i, 1});
		}
	}
...
State			loadInput(void) {
	...
	for (size_t i = 0; i < 4; ++i) {
		pb.rooms[i][1] = getAmphFromChar(line[getPosHallwayFromRoom(i) + 1]);
	}
